///connect4_playout(@state, max_playout_ms)
{
  var state = argument0,
  ...
  do {
    moves = connect4_generate_moves(state);
    // Game decided, return final result
    if (moves[0] == 0) {
      return connect4_final_playout(state);
    }
    // Game still ongoing, make random move
    else {
      connect4_apply_move(state, moves[irandom_range(1, moves[0])]);
    }
