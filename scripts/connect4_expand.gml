///connect4_expand(node)
{
  var node = argument0,
      node_state = connect4_deserialize(node[@MCTS_NODE.STATE]),
      moves = connect4_generate_moves(node_state);
  ...
  // Example: [2, <move A>, <child A>, <move B>, <child B>]
    ...
    for (var i = 1; i <= moves[0]; i++) {
      new_state = connect4_deserialize(node[@MCTS_NODE.STATE]);
      connect4_apply_move(new_state, moves[i]);
      children[(i-1)*2] = moves[i];
      children[((i-1)*2)+1] = MctsNode(connect4_serialize(new_state), 0, 0, undefined, undefined, undefined); 
    }
