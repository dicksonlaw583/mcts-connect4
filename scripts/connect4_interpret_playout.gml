///connect4_interpret_playout(node, playout)
// This takes a playout result (-1 = red wins, 0 = draw, 1 = yellow wins) and interprets it as a numeric reward in context of the player leading up to this node (-1 = I lose, 0 = We draw, 1 = I win)
...
// It's crucial to do this from the viewpoint of the right player. A bug here will result in "ostrich" behaviour (i.e. assuming that the opponent will always make mistakes)
{
  var node = argument0,
      playout = argument1,
      state = connect4_deserialize(node[@MCTS_NODE.STATE]);
  return -state[42]*playout[0];
}
